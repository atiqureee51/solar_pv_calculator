...
document.addEventListener('DOMContentLoaded', function() {
    const moduleModal = document.getElementById('moduleDetailsModal');
    const inverterModal = document.getElementById('inverterDetailsModal');
    ...
    window.showModuleDetails = function() {
        ...
        fetch(`/api/get_module_details?module=${encodeURIComponent(module)}`)
            ...
            .then(data => {
                ...
                const triggerButton = document.querySelector('[onclick="showModuleDetails()"]');
                
                moduleModal.addEventListener('shown.bs.modal', () => {
                    showModal(moduleModal);
                }, { once: true });
                
                moduleModal.addEventListener('hidden.bs.modal', () => {
    ...
    window.showInverterDetails = function() {
        ...
        fetch(`/api/get_inverter_details?inverter=${encodeURIComponent(inverter)}`)
            ...
            .then(data => {
                ...
                const bsModal = new bootstrap.Modal(inverterModal);
                const triggerButton = document.querySelector('[onclick="showInverterDetails()"]');
                
                inverterModal.addEventListener('shown.bs.modal', () => {
                    showModal(inverterModal);
                }, { once: true });
                
                inverterModal.addEventListener('hidden.bs.modal', () => {
                    hideModal(inverterModal);
                    document.getElementById('inverterDetailsContent').innerHTML = '';
                    if (triggerButton) triggerButton.focus();
                }, { once: true });
